...
class UserServiceImplTest {
    ...
    void whenCreateUser_thenReturnUserVO() {
        User userToSaveVO = UserFake.toFakeVO();
        UserEntity userToReturnSave = UserFake.toFake();

        Mockito.doReturn(userToReturnSave).when(repository).save(Mockito.any(UserEntity.class));
        Mockito.doReturn("encodedPass").when(passwordEncoder).encode(Mockito.any());

        User userAfterSave = userService.create(userToSaveVO);
    ...
    void whenFindByUsername_thenReturnUserVO() {
        UserEntity userToReturn = UserFake.toFake();

        Mockito.doReturn(Optional.of(userToReturn)).when(repository).findByUsername(Mockito.anyString());

        User userAfterFind = userService.findByUsername(userToReturn.getUsername());
    ...
    void whenFindById_thenReturnUserVO() {
        ...
        User userAfterFind = userService.findById(userToReturn.getId());
    ...
    void whenUpdateUser_andNotUpdatePassword_thenReturnUserVOSuccess() {
        ...
        UserEntity userToReturnFindById = UserFake.toFake();
        userToReturnFindById.setPassword(passwordUserSaved);

        User userToUpdate = UserFake.toFakeVO();
        userToUpdate.setId(userToReturnFindById.getId());
        userToUpdate.setPassword(null);

        Mockito.doReturn(userToReturnFindById).when(repository).save(Mockito.any(UserEntity.class));
    ...
    void saveTenantId_shouldUpdateUserTenantId_whenUserExists() {
        ...
        UserEntity user = UserFake.toFake();
        user.setId(userId);

        Mockito.when(repository.findById(userId)).thenReturn(Optional.of(user));
        Mockito.when(repository.save(Mockito.any(UserEntity.class))).thenReturn(user);
